...
def scrap_urls():
    ...
    req = requests.get(URL, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3'}, timeout=10)

    soup = BeautifulSoup(req.content, 'lxml')
    
    squads = {}

    for link in (soup.find_all('td', {"class":"left", "data-stat":["team", "country"]})):
        if link.find("a") != None:
...
def scrap_data(url_df, league):
    ...
    for squad in squadName:
        ...
        time.sleep(60) # wait for 60 seconds before sending the request
